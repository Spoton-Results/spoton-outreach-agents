...
Return: {
  ...
  try { return JSON.parse(await callClaude(SYSTEM, prompt, { quality: true })); } catch(e) { console.error("[Agent 04] Parse error:", e.message); return { hook: "Running commercial jobs in " + (prospect.city || prospect.state || "your area") + ",", hook_type: "inference", confidence: "low" }; }
}
